...
-   `Dialog` / `AlertDialog` / `Drawer`: Pinned header and footer chrome when the popup body overflows. `Dialog` and `Drawer` expose a new `Content` subcomponent that owns the scroll region (required for scrolling); render `Header` / `Footer` as siblings of `Content` to pin them to the popup edges, or nest them inside `Content` to scroll with the body. `AlertDialog.Popup` gains `stickyHeader` and `stickyFooter` props (default `true`) for its prescriptive title/actions chrome. Separator borders only show when there is off-screen content in that direction ([#77559](https://github.com/WordPress/gutenberg/pull/77559)).
